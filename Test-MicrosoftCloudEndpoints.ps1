...
function Test-NtpUdp123 {
    ...
    try {
        ...
        if ((Get-SafeCount $addresses) -eq 0) {
...
Write-Host "Reference records: $((Get-SafeCount $referenceUnique))"
Write-Host "Tested FQDN records: $((Get-SafeCount $results))"
Write-Host "Records with one or more failures: $((Get-SafeCount $failed))"
